...
doc.write('</form><br>');doc.close();doc.getElementById("aform"+idx).submit();this.postTime[idx]=new Date().getTime();}};this.sendDataToAgent=function(agentID,data){lmt12(plc1("Datapass is: "+data["Datapass"]+"; page marker: "+
...
if(this.isPersistentChat()||this.isPersistentWindow()){this.testForPersistentChat();}else{if(typeof NuanIJSFBridge!="undefined"){if(NuanIJSFBridge.getChatProgressState()){if(this.chat){this.chat.chatMgr=this;this.chat.monitorChatActive();}else{this.chatType="POPUP";var chatData={chatType:"POPUP",pC:false,ci:{},c2cToPersistent:false};this.chat=Chat.unmarshal(chatData);this.chat.monitorChatActive();}}else{this.removeChatInstance();}}else if(getIOSNativeSDKInstance()){if(isChatProgressInSDK){if(this.chat){this.chat.chatMgr=this;this.chat.monitorChatActive();}else{this.chatType="POPUP";var chatData={chatType:"POPUP",pC:false,ci:{},c2cToPersistent:false};this.chat=Chat.unmarshal(chatData);this.chat.monitorChatActive();}}else{this.removeChatInstance();}}else if(this.isV3Continue()){var now=(new Date()).getTime();var chatInterfaceData=this.getChatInterfaceData();var then=chatInterfaceData.lt;var initialTimeOut=chatInterfaceData.it;var chatWindowAppeared=chatInterfaceData.cwa;var isEngaged=chatInterfaceData.eng;if((CM47.chatSessionHelper.isContinueChat===false)&&(this.isChatTimedOut()||(!isEngaged&&(now-chatWindowAppeared)>initialTimeOut*1000)||(this.resetChatOnBrowserClose&&inu1(this.cip.z0())))){this.setDisconnectFlag(1,CHM.chatCloseReason.TIMEOUT);this.removeChatInstance();}
...
else{this.continueChatting();}}else if(getOSType()==="iOS"||getOSType()==="Android"){this.continueChatting();}}else if(this.isV3Active()){log("There is chat interface opened in other tabs, no c2c available or proactive will be displayed ");}else{this.removeChatInstance();log("Ended chat was closed");}}}catch(e){lmt12("CHM failure on onPageLanding: "+cf21(e),LOG_LEVELS.ERROR);}};GR34.prototype.isChatTimedOut=function(){return!!this.getChat()&&this.getChat().isTimedOut();};GR34.prototype.start=function(){if(this.started){return;}
...
function launchNativeSDK(chatparams){var agID=chatparams["agentGroupId"]||CHM.getChat().getAgentGroupID();var buID=chatparams["businessUnitID"]||getBusinessUnitID();var priority=CHM.getChat().rule.getPriority();var script=chatparams["scriptTreeId"]||CHM.getChat().getScriptID();var engageParams={siteID:""+Inq.getSiteID(),agID:""+agID,buID:""+buID,brID:""+getBrID(),brName:getBrName(),pageID:""+getPageID(),custID:""+CHM.getChat().getCustID()}
